...
type Region = Set Pos
data Edge = Edge Pos Direction deriving (Show, Eq)

makeGrid :: String -> Grid Char
makeGrid s = Map.fromList [ (P x y, color) | (line, y) <- zip (lines s) [1..]
                                       , (color, x) <- zip line [1..]]
neighbors :: Pos -> [Pos]
...
regionsOf grid = go (Map.keys grid) mempty mempty
  where
    ...
    go [] _ g = g
    go (p:ps) visited regions | Set.member p visited = go ps visited regions
    go (p:ps) visited regions = go ps (Set.union visited region) (region:regions)
    ...
    floodfillFrom p region
      ...
      where
        color = grid!?p
        recur = foldl (.) id $ floodfillFrom <$> filter ((==color).(grid!?)) (neighbors p)
...
sidesOf :: Region -> [Edge]
sidesOf region =
  let edges = edgesOf region
  in [Edge pos d | Edge pos d <- edges, (Edge (move (rot90 d) pos) d) `notElem` edges]
